\section{Herramientas utilizadas}

\subsection{Bibliotecas}

\begin{itemize}
\item \textbf{Stanford
Parser}\footnote{http://nlp.stanford.edu/software/lex-parser.shtml}.
Completo analizador de lenguaje natural desarrollado en la Universidad de
Stanford. (GPL)
\item \textbf{JSAP}\footnote{http://www.martiansoftware.com/jsap/}. Sencillo
analizador de opciones introducidas a través de la línea de comandos. (LGPL -
con excepciones)
\item \textbf{MySQL
Connector/J}\footnote{http://www.mysql.com/products/connector/j/}. Conjunto de
clases para poder operar con MySQL utilizando Java. (GPL)
\item \textbf{SAX}\footnote{http://www.saxproject.org/}. Un poderoso analizador
de ficheros XML, utilizando para leer y tratar los ficheros de entrada.
(Dominio público)
\end{itemize}

\subsection{Herramientas de construcción}

\begin{itemize}
\item \textbf{Ant}\footnote{http://ant.apache.org/}. La utilidad más
utilizada para crear guiones de construcción en XML para proyectos de software.
Especialmente útil para proyectos hechos en Java. (Apache)
\item \textbf{GNU Make}\footnote{http://www.gnu.org/software/make/}.
Alternativamente también se proporciona una vía para construir el proyecto
utilizando un Makefile. (GPL)
\end{itemize}

